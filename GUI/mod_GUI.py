...
for i in range(8):
    ...
    ax[i].set_yticks([0, 1])
    ax[i].tick_params(axis='y', colors='gray' if app._get_appearance_mode() == 'dark' else 'black')
    
    newax[i].set_ylim(-0.5, 1.5)
    newax[i].set_yticks([0, 1])
    newax[i].tick_params(axis='y', colors='gray' if app._get_appearance_mode() == 'dark' else 'black')

    ax[i].grid(True)
    ax[i].grid(color='gray', linestyle='-', linewidth=0.25)
    newax[i].grid(True)
    newax[i].grid(color='gray', linestyle='-', linewidth=0.25)

    # fig[i].set_tight_layout

    for spine in ax[i].spines.values():
        spine.set_edgecolor('gray' if app._get_appearance_mode() == 'dark' else 'black')
        spine.set_linewidth(0.4)

    for spine in newax[i].spines.values():
        spine.set_edgecolor('gray' if app._get_appearance_mode() == 'dark' else 'black')
        spine.set_linewidth(0.4)
...
def change_appearance(new_appearance_mode):
    ...
    for i in range(8):
        ...
        ax[i].tick_params(axis='y', colors='gray' if app._get_appearance_mode() == 'dark' else 'black')
        newax[i].tick_params(axis='y', colors='gray' if app._get_appearance_mode() == 'dark' else 'black')
        ax[i].tick_params(axis='x', colors='black' if app._get_appearance_mode() == 'dark' else 'white')
        newax[i].tick_params(axis='x', colors='black' if app._get_appearance_mode() == 'dark' else 'white')
