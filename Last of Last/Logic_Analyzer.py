    ...
    while (Runnung_Flag):
        if (DataDisplayed_Flag):
                    ...
                    if channels[i][j] == 0 and channels[i][j+1] == 1:
                        ...
                        for k in range(j+1, samples-1):
                            if channels[i][k] == 0 and channels[i][k+1] == 1:
                                time2 = times[k+1]
                                frequencies.append(
                                    calculate_freq(time1, time2))
                                break
                # frequencies = list(set(frequencies))

                frequencies = np.array(frequencies)
                # freq[i] = check_freq(frequencies)
                # frequencies = frequencies.astype(int)
                # frequencies = np.delete(frequencies, np.where(frequencies == 9478))
                            ...
                            if channels[i][k] == 1 and channels[i][k+1] == 0:
                                ...
                                for m in range(k+1, samples-1):
                                    if channels[i][m] == 0 and channels[i][m+1] == 1:
                                        time3 = times[m+1]
                                        duty_cycle[i] += ((time2-time1) /
                                                          (time3-time1))*100
                                        dc_counter += 1
                                        break
                                break
                duty_cycle[i] = duty_cycle[i] if dc_counter == 0 else duty_cycle[i]/dc_counter
                dc_counter = 0
            ...
            for j in range(int(samples / SPP)):
                for i in range(8):
                    draw_signal(
                        channels[i][j * SPP: j * SPP + SPP], times[j * SPP: j * SPP + SPP], i)
            channels = [[], [], [], [], [], [], [], []]
            DataDisplayed_Flag = True
            print("Displayed")
...
def draw_signal(signal, time_x, channelNumber):
    ...
    ax[channelNumber].set_xlim(time_x.min() - 1, time_x.max() + 1)
    newax[channelNumber].set_xlim(time_x.min() - 1, time_x.max() + 1)
